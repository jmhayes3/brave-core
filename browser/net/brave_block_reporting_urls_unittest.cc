...
const std::vector<const std::string> kNormalUrls({
    "https://brave.com/",
    "https://safebrowsing.google.com/safebrowsing",
    "https://safebrowsing.googleapis.com/v4",
});

const std::vector<const std::string> kReportingUrls({
...
class BraveBlockReportingUrlsHelperTest : public testing::Test {
 public:
  ...
  void check_url(const std::string& test_url,
...
void BraveBlockReportingUrlsHelperTest::check_url(const std::string& test_url,
                                                  const char* expected_url,
                                                  int expected_error) {
...
TEST_F(BraveBlockReportingUrlsHelperTest, PreserveNormalUrls) {
  for (const auto& url : kNormalUrls) {
    check_url(url, "", net::OK);
  }
}

TEST_F(BraveBlockReportingUrlsHelperTest, CancelReportingUrl) {
  for (const auto& url : kReportingUrls) {
    check_url(url, kInvalidUrl, net::ERR_ABORTED);
  }
}
